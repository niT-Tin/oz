...
/// False once the last terminal's deinit() freed `global_vts`; the SIGCHLD
/// handler must not touch the map then (a kill() racing the deinit would
/// otherwise iterate freed memory).
pub var global_vts_alive: bool = false;
...
pub fn deinit(self: *Terminal) void {
    ...
    if (self.thread) |*thread| {
        // drain pending child output first: the EOT write below would
            ...
            .osc => |osc| {
                ...
                switch (ps) {
                    0 => {
                        ...
                        // tryPush (drop when full): a blocking push here
                        // deadlocks deinit's thread.await when the queue
                        // fills while the main thread is joining
                        _ = try self.event_queue.tryPush(.{ .title_change = self.title.items });
                    },
                    7 => {
                        ...
                        _ = try self.event_queue.tryPush(.{ .pwd_change = self.working_directory.items });
                    },
                    else => {},
                }
            },
...
inline fn handleC0(self: *Terminal, b: ansi.C0) !void {
    switch (b) {
        ...
        .ENQ => {},
        .BEL => _ = try self.event_queue.tryPush(.bell),
