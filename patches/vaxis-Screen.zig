...
pub fn copyTo(self: *Screen, allocator: std.mem.Allocator, dst: *Screen) !void {
    // The back screen may carry scrollback rows ABOVE the visible window:
    // its buffer is rows*cols + scrollback cells, while the front screen is
    // exactly rows*cols. The visible window is the LAST dst rows (newest
    // output lands at the bottom of the back buffer as lines scroll), so
    // copy index-aligned cells starting at the back's tail — the naive
    // whole-buffer copy reads past dst.buf (index >= dst.len) once a line
    // scrolls into the scrollback area.
    const offset = self.buf.len -| dst.buf.len;
    dst.cursor = self.cursor;
    if (self.width > 0) dst.cursor.row = self.cursor.row -| @as(u16, @intCast(offset / self.width));
    for (self.buf[offset..], 0..) |cell, i| {
        if (!cell.dirty) continue;
        self.buf[offset + i].dirty = false;
